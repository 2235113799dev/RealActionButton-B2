...
- (NSArray *)specifiers {
    ...
    [test setProperty:@"play.fill" forKey:@"iconToken"]; [test setProperty:@"test.button" forKey:@"presentationKey"]; [test setProperty:@"开始测试" forKey:@"defaultTitle"]; [test setProperty:@"play.fill" forKey:@"defaultIcon"]; test->action=@selector(test:); [items addObject:test];
    [items addObject:[PSSpecifier groupSpecifierWithName:@"选择动作"]];
    NSArray *entries=@[@[@"基础动作",@"builtin",@"hand.tap.fill"],@[@"应用列表",@"app",@"app.badge.checkmark"],@[@"快捷指令",@"shortcut",@"square.stack.3d.up.fill"],@[@"链接列表",@"link",@"link"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else if ([[s propertyForKey:@"iconToken"] length]) { NSString *key=[s propertyForKey:@"presentationKey"],*fallback=[s propertyForKey:@"defaultIcon"] ?: [s propertyForKey:@"iconToken"];NSString *token=ABMCDisplayIconToken(key,fallback);ABMCApplyLargeIcon(cell,ABMCTintedIcon(token,nil));if(key.length)ABMCInstallPresentationLongPress(cell,self,key,ABMCDisplayTitle(key,[s name]),token,^{self->_specifiers=nil;[self reloadSpecifiers];}); }

...
- (void)viewDidLoad {[super viewDidLoad];_query=@"";self.tableView.rowHeight=44.0;self.tableView.estimatedRowHeight=44.0;self.navigationItem.rightBarButtonItem=[[UIBarButtonItem alloc]initWithTitle:@"清空" style:UIBarButtonItemStylePlain target:self action:@selector(clearSelectedActions)];[self refreshHeader];}

- (NSInteger)raw:(NSInteger)row {NSInteger n=0;for(NSUInteger i=0;i<sizeof(kActions)/sizeof(kActions[0]);i++)if((!_query.length||[kActions[i].title localizedCaseInsensitiveContainsString:_query])&&![ABMCSelectedActions(_key) containsObject:kActions[i].identifier])if(n++==row)return i;return NSNotFound;}
...
- (NSInteger)tableView:(UITableView *)t numberOfRowsInSection:(NSInteger)s{NSInteger n=0;for(NSUInteger i=0;i<sizeof(kActions)/sizeof(kActions[0]);i++)if((!_query.length||[kActions[i].title localizedCaseInsensitiveContainsString:_query])&&![ABMCSelectedActions(_key) containsObject:kActions[i].identifier])n++;return n;}

...
- (instancetype)initWithPreferenceKey:(NSString *)key { if ((self=[super initWithStyle:UITableViewStyleInsetGrouped])) { _preferenceKey=[key copy]; self.title=@"打开链接"; } return self; }
- (void)viewDidLoad {
    [super viewDidLoad]; _query=@"";
    UIView *header=[[UIView alloc]initWithFrame:CGRectMake(0,0,UIScreen.mainScreen.bounds.size.width,68)];
    UISearchBar *search=[[UISearchBar alloc]initWithFrame:CGRectInset(header.bounds,8,6)]; search.placeholder=@"搜索已存链接"; search.delegate=self; [header addSubview:search]; self.tableView.tableHeaderView=header;
...
- (void)editLink:(NSDictionary *)old {
    UIAlertController *alert=[UIAlertController alertControllerWithTitle:old?@"编辑链接":@"新增链接" message:@"第三项可填写 SF Symbol 名称，或应用 Bundle ID。留空会自动识别。" preferredStyle:UIAlertControllerStyleAlert];
    [alert addTextFieldWithConfigurationHandler:^(UITextField*f){f.placeholder=@"链接名称";f.text=old[@"title"]; }];
...
- (void)importClipboard { NSString*u=ABMCNormalizeURL(UIPasteboard.generalPasteboard.string);if(!u.length){UIAlertController*a=[UIAlertController alertControllerWithTitle:@"无法导入" message:@"剪贴板中没有有效的网页地址或 URL Scheme。" preferredStyle:UIAlertControllerStyleAlert];[a addAction:[UIAlertAction actionWithTitle:@"好" style:UIAlertActionStyleDefault handler:nil]];[self presentViewController:a animated:YES completion:nil];return;}[self editLink:@{ @"id":NSUUID.UUID.UUIDString,@"title":@"导入链接",@"url":u,@"icon":ABMCInferLinkIcon(u)}];}
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)path { NSDictionary*x=self.filteredLinks[path.row];UIContextualAction*edit=[UIContextualAction contextualActionWithStyle:UIContextualActionStyleNormal title:@"编辑" handler:^(UIContextualAction*a,UIView*v,void(^done)(BOOL)){[self editLink:x];done(YES);}];edit.backgroundColor=UIColor.systemBlueColor;UIContextualAction*remove=[UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive title:@"删除" handler:^(UIContextualAction*a,UIView*v,void(^done)(BOOL)){[_links removeObject:x];[self saveLinks];[self.tableView reloadData];done(YES);}];return[UISwipeActionsConfiguration configurationWithActions:@[remove,edit]];}

...
UIView *ABMCSelectedActionsBanner(NSString *preferenceKey) {
    NSArray *items=ABMCSelectedActions(preferenceKey);UIView *box=[[UIView alloc]initWithFrame:CGRectMake(0,0,UIScreen.mainScreen.bounds.size.width,82)];
    UILabel *caption=[[UILabel alloc]initWithFrame:CGRectMake(28,2,240,22)];caption.text=@"已选动作";caption.textColor=UIColor.secondaryLabelColor;caption.font=[UIFont systemFontOfSize:16];[box addSubview:caption];
    UIView *card=[[UIView alloc]initWithFrame:CGRectMake(20,27,box.bounds.size.width-40,48)];card.autoresizingMask=UIViewAutoresizingFlexibleWidth;card.backgroundColor=UIColor.secondarySystemGroupedBackgroundColor;card.layer.cornerRadius=15;[box addSubview:card];
    BOOL none=!items.count;NSMutableArray *titles=[NSMutableArray array];for(NSString *action in items)[titles addObject:ABMCBriefActionTitle(action)];NSString *title=none?@"无操作":[titles componentsJoinedByString:@" · "];
    UIImageView *image=[[UIImageView alloc]initWithImage:ABMCTintedIcon(none?@"nosign":items.count>1?@"square.grid.2x2.fill":@"hand.tap.fill",none?UIColor.systemRedColor:UIColor.systemBlueColor)];image.frame=CGRectMake(18,12,24,24);image.contentMode=UIViewContentModeScaleAspectFit;[card addSubview:image];
    UILabel *label=[[UILabel alloc]initWithFrame:CGRectMake(56,4,card.bounds.size.width-66,40)];label.autoresizingMask=UIViewAutoresizingFlexibleWidth;label.text=title;label.textColor=none?UIColor.systemRedColor:UIColor.systemBlueColor;label.font=[UIFont systemFontOfSize:18 weight:UIFontWeightRegular];label.numberOfLines=2;label.lineBreakMode=NSLineBreakByTruncatingTail;[card addSubview:label];return box;
}

...
static BOOL ABMCPanelUsesAppArtwork(NSString *action) { return [action hasPrefix:@"app:"]; }
static UIImage *ABMCPanelIcon(NSString *action) {
    if(ABMCPanelUsesAppArtwork(action)){NSString *bid=[action substringFromIndex:4];UIImage *image=nil;@try{image=[UIImage _applicationIconImageForBundleIdentifier:bid format:2 scale:UIScreen.mainScreen.scale];}@catch(__unused NSException *e){}if(image)return image;}

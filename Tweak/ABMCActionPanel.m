...
static UIWindow *ABMCPanelKeyWindow(void) { for(UIWindowScene *scene in UIApplication.sharedApplication.connectedScenes)if(scene.activationState==UISceneActivationStateForegroundActive)for(UIWindow *window in scene.windows)if(window.isKeyWindow)return window;return nil; }
...
- (void)showActions:(NSArray<NSString *> *)actions executor:(ABMCActionExecutor *)executor {
    if(!actions.count)return;dispatch_async(dispatch_get_main_queue(),^{
        ...
        self.previousKeyWindow=ABMCPanelKeyWindow();UIWindow *window=[[UIWindow alloc]initWithFrame:UIScreen.mainScreen.bounds];window.windowLevel=UIWindowLevelAlert+2;UIViewController *root=[UIViewController new];root.view.backgroundColor=UIColor.clearColor;window.rootViewController=root;self.window=window;[window makeKeyAndVisible];
        ...
        for(NSUInteger i=0;i<actions.count;i++){NSUInteger col=i%columns,row=i/columns;UIView *item=[[UIView alloc]initWithFrame:CGRectMake(padding+col*cellW,padding+row*cellH,cellW,cellH)];UIButton *button=[UIButton buttonWithType:UIButtonTypeCustom];button.frame=CGRectMake((cellW-58)*.5,0,58,58);button.layer.cornerRadius=29;button.clipsToBounds=YES;button.backgroundColor=[UIColor.secondarySystemFillColor colorWithAlphaComponent:.72];button.tag=i;CGFloat iconSide=MIN(48.0,MAX(12.0,ABMCPanelIconSize()))+4.0;UIImageView *iconView=[[UIImageView alloc]initWithImage:ABMCPanelIcon(actions[i])];iconView.frame=CGRectMake((58-iconSide)*.5,(58-iconSide)*.5,iconSide,iconSide);iconView.contentMode=UIViewContentModeScaleAspectFit;iconView.userInteractionEnabled=NO;[button addSubview:iconView];[button addTarget:self action:@selector(actionTapped:) forControlEvents:UIControlEventTouchUpInside];[item addSubview:button];UILabel *label=[[UILabel alloc]initWithFrame:CGRectMake(0,64,cellW,32)];label.text=ABMCPanelTitle(actions[i]);label.textAlignment=NSTextAlignmentCenter;label.font=[UIFont preferredFontForTextStyle:UIFontTextStyleFootnote];label.textColor=UIColor.labelColor;label.numberOfLines=2;label.lineBreakMode=NSLineBreakByTruncatingTail;[item addSubview:label];[card.contentView addSubview:item];}

...
- (void)showActions:(NSArray<NSString *> *)actions executor:(ABMCActionExecutor *)executor {
    if(!actions.count)return;dispatch_async(dispatch_get_main_queue(),^{
        ...
        card.frame=CGRectMake(cardInset,top,width,height);card.autoresizingMask=UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleBottomMargin;card.layer.cornerRadius=34;card.clipsToBounds=YES;[root.view addSubview:card];backdrop.card=card;self.card=card;self.cardTargetCenter=card.center;self.cardTargetBounds=card.bounds;self.islandCenter=CGPointMake(CGRectGetMidX(root.view.bounds),window.safeAreaInsets.top*.5);
        ...
        for(NSUInteger i=0;i<actions.count;i++){
            ...
            UILabel *label=[[UILabel alloc]initWithFrame:CGRectMake(0,67,cellW,27)];label.text=ABMCPanelTitle(action);label.textAlignment=NSTextAlignmentCenter;label.font=[UIFont systemFontOfSize:15 weight:UIFontWeightRegular];label.textColor=UIColor.labelColor;label.numberOfLines=2;label.lineBreakMode=NSLineBreakByWordWrapping;[item addSubview:label];[card.contentView addSubview:item];
        }
        UIPanGestureRecognizer *pan=[[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panned:)];[card addGestureRecognizer:pan];card.center=self.islandCenter;card.bounds=CGRectMake(0,0,126,37);card.alpha=.96;card.contentView.alpha=0;card.layer.cornerRadius=18.5;[UIView animateWithDuration:.30 delay:0 usingSpringWithDamping:.80 initialSpringVelocity:.72 options:UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionBeginFromCurrentState animations:^{card.center=self.cardTargetCenter;card.bounds=self.cardTargetBounds;card.layer.cornerRadius=34;card.contentView.alpha=1;} completion:nil];UIImpactFeedbackGenerator *feedback=[[UIImpactFeedbackGenerator alloc]initWithStyle:UIImpactFeedbackStyleLight];[feedback prepare];[feedback impactOccurred];
    });
}

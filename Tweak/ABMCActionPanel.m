...
- (void)dismissAnimated:(BOOL)animated completion:(dispatch_block_t)completion {if(!self.window){if(completion)completion();return;}UIWindow *window=self.window;UIView *card=self.card;void(^done)(BOOL)=^(__unused BOOL finished){UIWindow *host=self.hostWindow;window.hidden=YES;window.rootViewController=nil;self.window=nil;self.card=nil;self.content=nil;self.actions=nil;self.hostWindow=nil;if(host&&!host.hidden)[host makeKeyWindow];if(completion)completion();};if(animated){UIView *content=self.content;CGFloat exitY=-MIN(54.0,CGRectGetMinY(card.frame)+18.0);[UIView animateWithDuration:.20 delay:0 options:UIViewAnimationOptionCurveEaseIn|UIViewAnimationOptionBeginFromCurrentState animations:^{content.alpha=0;card.transform=CGAffineTransformMakeTranslation(0,exitY);card.alpha=0;} completion:done];}else done(YES);}
@end

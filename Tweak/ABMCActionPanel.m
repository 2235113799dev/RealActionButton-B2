...
- (void)showActions:(NSArray<NSString *> *)actions executor:(ABMCActionExecutor *)executor {
    if(!actions.count)return;dispatch_async(dispatch_get_main_queue(),^{
        ...
        UIPanGestureRecognizer *pan=[[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panned:)];[card addGestureRecognizer:pan];CGPoint target=card.center;CGFloat scale=.28;CGAffineTransform launch=CGAffineTransformConcat(CGAffineTransformMakeScale(scale,scale),CGAffineTransformMakeTranslation((self.islandCenter.x-target.x)/scale,(self.islandCenter.y-target.y)/scale));card.transform=launch;card.alpha=0;[UIView animateWithDuration:.28 delay:0 usingSpringWithDamping:.76 initialSpringVelocity:.86 options:UIViewAnimationOptionCurveEaseOut|UIViewAnimationOptionBeginFromCurrentState animations:^{card.transform=CGAffineTransformIdentity;card.alpha=1;} completion:nil];UIImpactFeedbackGenerator *feedback=[[UIImpactFeedbackGenerator alloc]initWithStyle:UIImpactFeedbackStyleLight];[feedback prepare];[feedback impactOccurred];
    });
}
...
- (void)dismissAnimated:(BOOL)animated completion:(dispatch_block_t)completion {if(!self.window){if(completion)completion();return;}UIWindow *window=self.window;UIView *card=self.card;void(^done)(BOOL)=^(__unused BOOL finished){window.hidden=YES;UIWindow *host=self.hostWindow;self.window=nil;self.card=nil;self.actions=nil;self.hostWindow=nil;if(host)[host makeKeyWindow];if(completion)completion();};if(animated){CGPoint target=card.center;CGFloat scale=.28;CGAffineTransform collapse=CGAffineTransformConcat(CGAffineTransformMakeScale(scale,scale),CGAffineTransformMakeTranslation((self.islandCenter.x-target.x)/scale,(self.islandCenter.y-target.y)/scale));[UIView animateWithDuration:.20 delay:0 options:UIViewAnimationOptionCurveEaseIn|UIViewAnimationOptionBeginFromCurrentState animations:^{card.transform=collapse;card.alpha=0;} completion:done];}else done(YES);}
@end
